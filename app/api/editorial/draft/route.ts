...
function assertFinalSourceIntegrity(article: { sourceNotes?: FinalSourceNote[] }, story: RawStoryInput) {
  ...
  const validNotes = notes.filter((note) => { const sourceId = String(note.sourceId ?? "").trim(); const publisher = String(note.publisher ?? "").trim(); const url = String(note.url ?? "").trim(); const source = sourceById.get(sourceId); return Boolean(source && publisher && /^https?:\/\//i.test(url) && String(source.publisher ?? "").trim().toLowerCase() === publisher.toLowerCase()); });
  const publishers = new Set(validNotes.map((note) => String(note.publisher).trim().toLowerCase()));
  if (notes.length < 2 || validNotes.length !== notes.length || publishers.size < 2) throw new Error(`Final source-integrity gate failed: ${notes.length} source notes, ${validNotes.length} mapped to supplied evidence, ${publishers.size} distinct publishers; minimum is 2 valid notes from 2 publishers.`);
}
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    console.error("Editorial draft pipeline failed", { requestId, durationMs: Date.now() - startedAt, error: error instanceof Error ? error.message : "Editorial draft pipeline failed" });
    return jsonResponse({ error: error instanceof Error ? error.message : "Editorial draft pipeline failed", requestId }, { status: 500 });
  }
}
