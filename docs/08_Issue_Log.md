...
| ACCRED-001 | Implemented | Critical | Analytics / Accreditation | Build durable evidence of publishing cadence, traffic, engagement and search visibility. | Analytics and evidence-pack architecture was not implemented. | Pending PR | Consent-aware GA4/GTM loader and newsroom metadata implemented on `feat/newsroom-foundation`; provider IDs still require Vercel environment configuration | Pending build, merge, deployment, consent-flow and analytics-event verification | — |
