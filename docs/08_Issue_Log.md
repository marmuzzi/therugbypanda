...
| AUTO-003 | In Progress | Critical | Scheduling / Orchestration | Prepare eight review-ready articles by 08:00 Europe/Dublin daily. | Persistent Make.com orchestration is not configured. | #47–#54 foundation | Make MCP connection verified; production orchestration scenarios not yet configured | Pending repeated successful daily runs, retries and deadline monitoring | — |
| NOTIFY-001 | Pending Verification | High | Editorial Notifications | Email `editor@therugbypanda.ie` when an article enters the Editorial Review queue. | Application webhook foundation is merged, but downstream Make routing and production configuration are incomplete. | #82 | Application-side webhook merged; Make MCP Health Check verified; production Make scenario and Vercel environment configuration pending | Pending controlled submit, one delivered email, duplicate-event protection and failure-path tests | — |
...
| ACCRED-001 | Implemented | Critical | Analytics / Accreditation | Build durable evidence of publishing cadence, traffic, engagement and search visibility. | Analytics and evidence-pack architecture was not implemented. | #76, pending dashboard PR | Consent-aware GA4/GTM loader is merged; Sanity Newsroom Dashboard implemented with publishing, review, rejection, replacement, original-photo and competition-coverage metrics | Dashboard preview build passed; pending merge, production deployment, authenticated Studio verification and provider-ID analytics-event verification | — |
...
PR #64 introduced a broken Editorial Review component. PR #66 repaired the build and Tool contract. PR #67 completed the maintainability refactor and QA improvements. PR #81 added real Sanity-backed website search. PR #82 added the application-side review-ready notification webhook. Current GitHub `main` is `39f8e1ddc7ff3d31f07bc3879f2dbeac031360e7`; fresh Vercel verification is required for the current session.

## Make.com integration status

- The Make.com MCP toolbox is connected.
- The temporary Health Check tool executed successfully on 26 July 2026.
- Production-ready Rugby Panda tools and scenarios are not yet exposed through the toolbox.
- NOTIFY-001 remains incomplete until scenario configuration, environment configuration, email delivery and deduplication are verified.
