...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| PUB-003 | Pending Verification | High | Editorial Automation / Media | Apply human-reviewed Editorial Image metadata through a controlled dry-run-first importer. | #40 | Merged to main commit `cd442f47eee73237f935bdad9848f789ca7c618d`; workflow available | Example-file dry run succeeded technically and correctly rejected nonexistent ID `media-candidate-example`; no Sanity mutation occurred. Real canonical-ID dry run and explicit apply remain pending. | — |
| PUB-002 | Pending Verification | High | Editorial Automation / Media | Generate reviewable metadata suggestions for Editorial Images without mutating Sanity. | #39 | Merged to main commit `4fe2ace1db1a61f24d3e9cbb21b49c786dd6c4b8`; workflow available | Artifact reviewed: 40 records, 18 with suggestions, 11 metadata-review-ready, 1 rights-review-required, 22 complete, 6 inactive. Draft/published pairs identified; canonical-record filtering remains required. | — |
...
- **Related PRs:** #40.
- **Deployment status:** Merged to `main` at `cd442f47eee73237f935bdad9848f789ca7c618d`; workflow available.
- **Verification status:** The first dry run used `review-decisions.example.json` and processed the placeholder ID `media-candidate-example`. It correctly returned `Editorial Image not found`, applied zero changes and made no Sanity mutation. Pending: create a real reviewed decision file using canonical production IDs, run `apply_changes=false`, inspect the report, then run an explicit apply only if clean.
