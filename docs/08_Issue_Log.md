...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| DOC-003 | Closed | High | Documentation | Reconcile core source-of-truth docs through Monday go-live state. | Core docs stopped at 20 Aug while #195-#230 changed production state. | #231 | Merged/deployed | Repository reconciliation verified | 2026-08-24 |
...
| MEDIA-009 | In Progress | Critical | Media / Assignment | Prevent semantically wrong and duplicate morning-package images; support relevant inline subject photography. | Article assignment used broad category/token matching, did not reserve assets across a five-story package, and supported only one automatic article image. | current `fix/editorial-quality-image-assignment` branch | Implemented on branch | Preview, production five-story assignment audit and inline-image proof pending | — |
...
| AUTO-004 | Pending Verification | Critical | Editorial Automation / Quality | Produce a complete five-story Draft Ready/originality-safe package. | Earlier timeout, originality overlap, metadata-length and now escaped-formatting/image-assignment failures blocked trustworthy 5/5. | #195-#200, #219, #220 plus current quality/media branch | Quality fixes merged/deployed; new fixes implemented on branch | Hard clean 5/5 production result and side-by-side inspection pending | — |
...
| EDIT-005 | In Progress | Critical | Editorial Formatting | No escaped/raw Markdown or formulaic generic headings may reach Draft Ready. | Structured generator returned Markdown-like text inside plain paragraph strings; Draft Ready did not detect it, and explainer guidance encouraged “why this matters now” phrasing. | current `fix/editorial-quality-image-assignment` branch | Implemented on branch | Preview + fresh production draft inspection pending | — |
| WEB-010 | Pending Verification | High | Frontend / Article Layout | Public article pages need genuinely different editorial presentations. | Previous three variants were slug-hash based and structurally similar. | #229 | Merged/deployed | Representative live desktop/mobile variants pending | — |
| WEB-011 | Pending Verification | High | Frontend / Homepage | Homepage needs editorial hierarchy rather than identical article cards. | Uniform card grid flattened story importance/type. | #230 | Merged/deployed | Multi-story live homepage desktop/mobile proof pending | — |
...
| CMS-005 | Pending Verification | Critical | CMS / Newsroom | Draft articles opened from Newsroom Dashboard must route to the Sanity editor without fatal error. | Custom dashboard used unsupported query-string edit-intent syntax. | #232 | Merged; production deployment initiated | Authenticated Dashboard -> draft -> edit/save/reload verification pending | — |
| CMS-002 | In Progress | Critical | CMS / Visual Content | Correct relevant images on launch/existing articles and remove legacy bad assignments. | Old drafts and broad matching rules allowed wrong assignments. | #49, #176, #192, #230 plus current media branch | New strict semantic/package rules implemented on branch | Legacy cleanup and fresh five-story audit pending | — |
...
- #219 metadata-only Draft Ready repair is merged/deployed; #220 controlled five-story verification trigger is merged; hard clean 5/5 production evidence is still required.
- #232 fixes Newsroom Dashboard edit-intent routing; authenticated Sanity verification remains required.
- A fresh owner review on 24 August exposed escaped Markdown, generic opening/final headings, duplicate package images and semantically wrong image assignments. EDIT-005/MEDIA-009 are now launch-critical and must be verified on a new five-story package before AUTO-004 closes.
...
- #229 article presentation and #230 homepage hierarchy are merged/deployed; representative production visual verification remains.

## Owner-help boundary

Do not require owner help for routine implementation, merges or clear image approvals. Escalate only genuinely ambiguous rights/relevance cases (target <=5%), unavailable external account credentials/authorization, authenticated owner verification steps, final editorial judgement and final go-live acceptance.
