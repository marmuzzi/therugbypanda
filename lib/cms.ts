...
function mapArticleSummary(article: SanityArticleSummary): ArticleSummary {
  ...
  const image = article.useBrandImage ? BRAND_ARTICLE_IMAGE : assignedImage;
  ...
  return {
    ...
    image,
    imageAlt: article.useBrandImage ? "The Rugby Panda" : article.featuredImage?.alt,
  };
}
...
export function getFeaturedImage(article: CmsArticle): FeaturedImage | undefined {
  if (article.useBrandImage) {
    return {
      src: BRAND_ARTICLE_IMAGE,
      alt: "The Rugby Panda",
    };
  }
  ...
  return {
    ...
    credit:
      [article.featuredImage?.photographer, article.featuredImage?.source]
        .filter(Boolean)
...
export function portableTextToSections(body: CmsArticle["body"]): ArticleBodySection[] {
  ...
  for (const block of body) {
    ...
    else sections.push({ paragraphs: [text] });
  }
...
export async function getSectionLinks(): Promise<SectionLink[]> {
  const categories = await sanityFetch<Array<{ title: string; slug: string }>>({
  ...
  const deduped = categoryLinks.filter(
    (category, index, all) => all.findIndex((item) => item.href === category.href) === index,
  );
  const sorted = deduped.sort((a, b) => {
    const ai = primarySectionOrder.indexOf(a.label);
    const bi = primarySectionOrder.indexOf(b.label);
    if (ai !== -1 || bi !== -1) {
