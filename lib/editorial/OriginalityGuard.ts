...
function longestSharedContiguousRun(left: string[], right: string[]) {
  if (left.length === 0 || right.length === 0) return { length: 0, phrase: undefined as string | undefined };
  const previous = new Array(right.length + 1).fill(0) as number[];
  let longest = 0;
  let longestLeftEnd = 0;
  ...
  return {
    length: longest,
    phrase: longest > 0 ? left.slice(longestLeftEnd - longest, longestLeftEnd).join(" ") : undefined,
  };
}
...
export function assessArticleOriginality(
  ...
  const findings = protectedSources.map((source) => {
    ...
    const sharedRun = longestSharedContiguousRun(articleWords, sourceWords);

    if (sharedRun.length > MAX_LONGEST_SHARED_RUN) {
      reasons.push(`${source.publisher} (${source.id}) shares ${sharedRun.length} consecutive normalized words with the draft.`);
    }
    ...
    return {
      ...
      longestSharedRun: sharedRun.length,
      longestSharedPhrase: sharedRun.phrase,
      sixGramCoverage,
      matchedSixGrams,
    };
  });
