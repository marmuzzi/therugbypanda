    ...
    issues: {
      ...
      items: {
        ...
        properties: {
          ...
          category: { type: "string", enum: ["facts", "originality", "structure", "style", "formatting", "clarity", "rugby-value"] },
          message: { type: "string" },
...
function outputText(payload: ResponsesPayload): string {
  ...
  for (const item of payload.output ?? []) {
    ...
    for (const part of item.content ?? []) {
      if (part.type === "refusal" && part.refusal) throw new Error(`Publication review refused: ${part.refusal}`);
...
async function structuredCall<T>(input: string, schema: object, name: string, timeoutMs = REVIEW_TIMEOUT_MS): Promise<{ value: T; usage?: ResponsesPayload["usage"] }> {
  ...
  try {
    ...
    return { value: JSON.parse(outputText(payload)) as T, usage: payload.usage };
  } finally {
...
function reviewInput(article: GeneratedArticleDraft, editorial: EditorialBrainResult, story: RawStoryInput, pass: 1 | 2) {
  return JSON.stringify({
    assignment: `Publication Review #${pass} for a Rugby Panda draft. Judge whether it is genuinely ready for the owner to read.`,
    article,
    editorialBrief: editorial.brief,
    factLedger: editorial.factLedger,
    sourceProvenance: story.sourceRecords.map(({ id, publisher, title, url, publishedAt, isPrimarySource }) => ({ id, publisher, title, url, publishedAt, isPrimarySource })),
    winningKeys: [
      "diversity of angle and structure",
      "original synthesis rather than source-shaped rewriting",
      "genuine rugby curiosity",
      "facts grounded in supported stats, datapoints and names",
      "useful tactical or strategic rugby insight where evidence supports it",
      "natural human newsroom voice",
      "a unique reason for this article to exist",
    ],
    hardRules: [
      "No invented facts, numbers, quotes or causal claims.",
      "No raw or escaped Markdown markers.",
      "No generic headings such as Why this matters now, What you need to know, The bigger picture or What happens next.",
      "The opening starts with the story itself.",
      "Do not reward verbosity, formulaic sectioning, repetitive bolding, generic conclusions or AI-sounding rhetorical patterns.",
      "Only critical/high issues block publication readiness. Medium/low observations should be specific and genuinely useful, not stylistic nitpicking.",
    ],
  });
}
...
function correctionInput(article: GeneratedArticleDraft, review: PublicationReview, editorial: EditorialBrainResult) {
  return JSON.stringify({
    assignment: "Make one bounded publication-quality correction pass. Return the complete corrected article.",
    article,
    reviewIssues: review.issues,
    factLedger: editorial.factLedger,
    constraints: [
      "Fix every critical/high issue and worthwhile medium issue without turning the article into a different story.",
      "Use only supported facts in the fact ledger; introduce no new names, numbers, quotes or claims.",
      "Preserve originality and the article's assigned editorial identity.",
      "Prefer natural prose changes over adding headings, lists, bold markers or explanatory boilerplate.",
      "Generated strings are structured plain text, not Markdown.",
      "Do not add Why this matters now, What you need to know, The bigger picture or What happens next headings.",
      "Keep sourceNotes and internal disclosure accurate.",
    ],
  });
}

function blockingIssues(review: PublicationReview) {
  return review.issues.filter((issue) => issue.severity === "critical" || issue.severity === "high");
}

function rawStoryMaterial(story: RawStoryInput) {
...
function assertDeterministicGates(article: GeneratedArticleDraft, story: RawStoryInput) {
  ...
  const originality = assessArticleOriginality(
    article,
    story.sourceRecords,
    rawStoryMaterial(story) ? [{ id: `${story.id}:raw-story-material`, publisher: "Acquired story material", text: rawStoryMaterial(story) }] : [],
  );
  if (!quality.passed || !originality.passed) {
    throw new Error(`Post-review deterministic gate failed: ${[
...
export async function runPublicationReviewCycle(
  article: GeneratedArticleDraft,
  editorial: EditorialBrainResult,
  story: RawStoryInput,
): Promise<PublicationReviewCycleResult> {
  ...
  const review1 = first.value;
  const needsCorrection = review1.verdict === "revise" || blockingIssues(review1).length > 0;
  ...
  const review2 = second.value;
  const blocking = blockingIssues(review2);
  if (blocking.length > 0) {
    throw new Error(`Publication Review #2 rejected article: ${blocking.map((issue) => `${issue.category}: ${issue.message}`).join(" ")}`);
  }

  assertDeterministicGates(correctedArticle, story);
  console.info("Publication review cycle completed", {
    inputId: editorial.inputId,
    model: REVIEW_MODEL,
    corrected: needsCorrection,
    review1Issues: review1.issues.length,
    review1Blocking: blockingIssues(review1).length,
    review2Issues: review2.issues.length,
    review2Blocking: blocking.length,
    review1Usage: first.usage,
    review2Usage: second.usage,
  });
