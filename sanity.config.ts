...
const newsroomDashboardTool = definePlugin({
  name: "newsroom-dashboard-tool",
  tools: [{ name: "newsroom-dashboard", title: "Newsroom Dashboard", component: NewsroomDashboardTool }],
});

const editorialReviewTool = definePlugin({
  name: "editorial-review-tool",
  tools: [{ name: "editorial-review", title: "Editorial Review", component: EditorialReviewTool }],
});

const editorialQaTool = definePlugin({
  name: "editorial-qa-tool",
  tools: [{ name: "editorial-qa", title: "Editorial QA", component: EditorialQaTool }],
});

const editorialImageReviewTool = definePlugin({
  name: "editorial-image-review-tool",
  tools: [{ name: "editorial-image-review", title: "Image Review", component: EditorialImageReviewTool }],
});

const brandAssetReviewTool = definePlugin({
  name: "brand-asset-review-tool",
  tools: [{ name: "brand-asset-review", title: "Brand Review", component: BrandAssetReviewTool }],
});
            ...
            S.listItem()
              ...
              .child(
                S.list()
                  .title("Brand Assets")
                  .items([
                    S.listItem().title("Needs Review").schemaType("brandAsset").child(
                      S.documentList().title("Brand Assets Needing Review").schemaType("brandAsset")
                    ...
                    S.listItem().title("Approved").schemaType("brandAsset").child(
                      S.documentList().title("Approved Brand Assets").schemaType("brandAsset")
                        .filter('_type == "brandAsset" && lifecycleStatus in ["approved", "Approved"] && approvedForEditorialUse == true')
                        .defaultOrdering([{ field: "title", direction: "asc" }]),
                    ),
                    S.listItem().title("Rejected").schemaType("brandAsset").child(
                      S.documentList().title("Rejected Brand Assets").schemaType("brandAsset")
                        .filter('_type == "brandAsset" && lifecycleStatus in ["rejected", "Rejected"]')
                        .defaultOrdering([{ field: "_updatedAt", direction: "desc" }]),
                    ),
                    S.listItem().title("Active Brands").schemaType("brandAsset").child(
                      S.documentList().title("Active Brand Assets").schemaType("brandAsset")
                        .filter('_type == "brandAsset" && (!defined(status) || status == "active")')
                        .defaultOrdering([{ field: "title", direction: "asc" }]),
                    ),
                    S.listItem().title("Teams").schemaType("brandAsset").child(
                      S.documentList().title("Team Brand Assets").schemaType("brandAsset")
                        .filter('_type == "brandAsset" && brandType == "team"')
                        .defaultOrdering([{ field: "title", direction: "asc" }]),
                    ),
                    S.listItem().title("Competitions & Leagues").schemaType("brandAsset").child(
                      S.documentList().title("Competition & League Brand Assets").schemaType("brandAsset")
                        .filter('_type == "brandAsset" && brandType in ["competition", "league", "tournament"]')
                        .defaultOrdering([{ field: "title", direction: "asc" }]),
                    ),
                    S.listItem().title("Unions & Governing Bodies").schemaType("brandAsset").child(
                      S.documentList().title("Union Brand Assets").schemaType("brandAsset")
                        .filter('_type == "brandAsset" && brandType == "union"')
                        .defaultOrdering([{ field: "title", direction: "asc" }]),
                    ),
                    S.listItem().title("Needs Rights Review").schemaType("brandAsset").child(
                      S.documentList().title("Brand Assets Needing Rights Review").schemaType("brandAsset")
                    ...
                    S.divider(),
                    S.listItem().title("All Brand Assets").schemaType("brandAsset").child(
                      S.documentTypeList("brandAsset").title("All Brand Assets").defaultOrdering([{ field: "title", direction: "asc" }]),
                    ),
                  ]),
              ),
            S.divider(),
            S.listItem()
              ...
              .child(
                S.list()
                  .title("Editorial Images")
                  .items([
                    S.listItem().title("Needs Review").schemaType("editorialImage").child(
                      S.documentList().title("Needs Review").schemaType("editorialImage")
                    ...
                    S.listItem().title("Approved").schemaType("editorialImage").child(
                      S.documentList().title("Approved Editorial Images").schemaType("editorialImage")
                        .filter('_type == "editorialImage" && lifecycleStatus in ["approved", "Approved"]')
                        .defaultOrdering([{ field: "_updatedAt", direction: "desc" }]),
                    ),
                    S.listItem().title("Published").schemaType("editorialImage").child(
                      S.documentList().title("Published Editorial Images").schemaType("editorialImage")
                        .filter('_type == "editorialImage" && lifecycleStatus in ["published", "Published"]')
                        .defaultOrdering([{ field: "_updatedAt", direction: "desc" }]),
                    ),
                    S.listItem().title("Rugby Panda Originals").schemaType("editorialImage").child(
                      S.documentList().title("Rugby Panda Originals").schemaType("editorialImage")
                        .filter('_type == "editorialImage" && sourceClassification in ["the-rugby-panda-original", "The Rugby Panda Original"]')
                        .defaultOrdering([{ field: "_updatedAt", direction: "desc" }]),
                    ),
                    S.listItem().title("Rejected").schemaType("editorialImage").child(
                      S.documentList().title("Rejected Editorial Images").schemaType("editorialImage")
                        .filter('_type == "editorialImage" && lifecycleStatus in ["rejected", "Rejected"]')
                        .defaultOrdering([{ field: "_updatedAt", direction: "desc" }]),
                    ),
                    S.divider(),
                    S.listItem().title("All Editorial Images").schemaType("editorialImage").child(
                      S.documentTypeList("editorialImage").title("All Editorial Images").defaultOrdering([{ field: "_updatedAt", direction: "desc" }]),
                    ),
                  ]),
              ),
            S.divider(),
