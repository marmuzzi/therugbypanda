...
const importLimit = Math.max(1, Number.parseInt(process.env.BULK_IMAGE_IMPORT_LIMIT ?? "2000", 10));
const perScope = Math.max(10, Math.min(120, Number.parseInt(process.env.BULK_IMAGE_PER_SCOPE ?? "70", 10)));
...
function photoType(text) { const t = text.toLowerCase(); if (/stadium|ground|arena/.test(t)) return "stadium"; if (/scrum/.test(t)) return "scrum"; if (/lineout|line-out/.test(t)) return "lineout"; if (/tackle/.test(t)) return "tackle"; if (/training|warm.?up/.test(t)) return "training"; if (/crowd|supporter|fan/.test(t)) return "crowd"; if (/portrait|headshot/.test(t)) return "portrait"; return "action"; }
function category(scope) { if (scope === "Ireland Women") return "womens-rugby"; if (["Ireland","Six Nations","New Zealand","South Africa","Australia","Argentina","England","Scotland","Wales","France","Italy","Fiji","Japan","Samoa","Tonga"].includes(scope)) return "international"; if (scope.startsWith("Rugby ")) return "rugby-culture"; return "club-rugby"; }
...
async function discover(scope, query) {
  ...
  const response = await fetch(url, { headers:{ "User-Agent":"TheRugbyPanda/1.0 (bulk open-licence rugby library; hello@therugbypanda.ie)" } });
  ...
  return Object.values(payload?.query?.pages ?? {}).map((page) => {
    ...
    const relevant = rugbyEvidence.test(evidence) && !blocked.test(evidence);
    const usable = /^image\/(jpeg|png|webp)$/i.test(mime) && width >= 1200 && height >= 600;
    return { scope, query, title, description, categories, sourcePage:info.descriptionurl, imageUrl:info.url, width, height, mime, licence, licenceUrl, creator, rightsClear, relevant, usable, evidence };
  }).filter((x) => x.sourcePage && x.imageUrl && x.rightsClear && x.relevant && x.usable);
}

const discovered = [];
for (const [scope, query] of scopes) {
  try { const items = await discover(scope, query); discovered.push(...items); console.log(`DISCOVER ${scope}: ${items.length}`); }
...
async function importOne(item) {
  ...
  try {
    ...
    await client.createOrReplace({
      _id:`editorialImage-wikimedia-bulk-${hash}`, _type:"editorialImage", title:item.title,
      ...
      altText:caption, caption, lifecycleStatus:"approved", usageApproved:true, editorialCategory:category(item.scope), photoType:photoType(item.evidence), editorialValue:"historical", editorialRating:3,
      suggestedUse:["article-header","homepage-card","gallery"], team:item.scope.startsWith("Rugby ") || ["Six Nations","URC","Champions Cup","Challenge Cup"].includes(item.scope) ? undefined : item.scope,
      ...
      licence:item.licence, licenceUrl:item.licenceUrl, rightsNotes:`Open-licence Wikimedia asset imported by deterministic rugby relevance and rights gate. Preserve attribution. ${publicCredit}`,
      ...
      width:item.width, height:item.height, orientation:item.width >= item.height ? "landscape" : "portrait", searchKeywords:[item.scope,"rugby"], importedAt:new Date().toISOString()
    });
    existingSources.add(item.sourcePage); imported += 1; coverage[item.scope] = (coverage[item.scope] ?? 0) + 1;
...
const summary = { generatedAt:new Date().toISOString(), discovered:discovered.length, uniqueCandidates:deduped.length, queued:queue.length, imported, skipped, failed, coverage, openAiCalls:0 };
console.log(JSON.stringify(summary, null, 2));
if (process.env.GITHUB_STEP_SUMMARY) {
  const fs = await import("node:fs/promises");
  await fs.appendFile(process.env.GITHUB_STEP_SUMMARY, `## Bulk Rugby Image Import\n\n- Discovered: ${summary.discovered}\n- Unique rights-safe candidates: ${summary.uniqueCandidates}\n- Imported: ${summary.imported}\n- Failed: ${summary.failed}\n- OpenAI calls: 0\n`);
}
