...
const TEAM_OR_COMPETITION = new Set([
  ...
  "Ireland", "Ireland Women", "Leinster", "Munster", "Ulster", "Connacht", "England",
  "Scotland", "Wales", "France", "Italy", "Argentina", "Pumas", "Fiji", "Japan",
  "Samoa", "Tonga", "United Rugby Championship", "Six Nations", "Champions Cup",
  "Challenge Cup", "World Cup", "Rugby Championship", "Red Roses", "WXV Global",
  "WXV Global Series", "Global Series", "Western Force",
]);
const NON_PERSON_NAMES = new Set([
  "Planet Rugby", "Irish Rugby", "Rugby Pass", "RugbyPass Ireland", "Business Post",
  "The Irish Times", "The Guardian", "United Rugby", "Rugby Football", "Football Union",
  "United Rugby Championship", "Irish Independent", "Connacht Rugby", "Munster Rugby",
  "Leinster Rugby", "Ulster Rugby", "England Rugby", "Red Roses", "WXV Global",
  "WXV Global Series", "Global Series", "Wallaroos Global", "Western Force", "Force Force",
  "Roster Confirmed", "ROSTER CONFIRMED", "WXV Match", "WXV Series", "Two-time Olympic",
]);
...
const EXPLICIT_NON_RUGBY_TITLE = /\b(?:nations league|premier league|champions league|soccer|gaelic football|gaa|hurling|camogie|golf|boxing|formula one|f1)\b/i;
...
function hasPersonCollision(titleEvidence, factEvidence) { const primary = personNames(titleEvidence).map(canonicalPerson); const facts = personNames(factEvidence).map(canonicalPerson); return primary.some((person) => person.last && facts.some((fact) => fact.last === person.last && fact.first && person.first && fact.first !== person.first)); }
function titleSupportsFactPerson(titleEvidence, person) { const title = normalise(titleEvidence); const canonical = canonicalPerson(person); if (!title || !canonical.last) return false; if (canonical.full && title.includes(canonical.full)) return true; return canonical.last.length >= 4 && title.split(/\s+/).includes(canonical.last); }
...
function sourceMentionsPerson(source, person) { const text = normalise([source?.title, source?.excerpt, source?.bodyText].map(clean).filter(Boolean).join(" ")); const canonical = canonicalPerson(person); return Boolean(text && canonical.full && text.includes(canonical.full)); }
...
async function countStrictRetainedDrafts() {
  ...
  const packageDate = raw.packageDate || operationalDate(), prefix = `current-${packageDate}-*`;
...
function assess(candidate) {
  ...
  const sourceText = editorialSources.flatMap((source) => [source.title, source.excerpt, source.bodyText]).map(clean).filter(Boolean), evidence = [candidate.title, candidate.summary, candidate.editorialPosition?.subject, candidate.editorialPosition?.development, ...distinctFacts, ...sourceText].map(clean).filter(Boolean).join(" "), titleEvidence = [candidate.title, candidate.editorialPosition?.subject, candidate.editorialPosition?.development].map(clean).filter(Boolean).join(" ");
  const names = personNames(evidence), directNames = personNames(titleEvidence), factPeople = personNames(factEvidence), corroboratedPeople = names.filter((name) => editorialSources.filter((source) => sourceMentionsPerson(source, name)).length >= 2), distinctPublishers = new Set(editorialSources.map((source) => clean(source.publisher || source.name).toLowerCase()).filter(Boolean));
  const hasConcreteMarker = SCORE_OR_NUMBER.test(evidence) || MONTH.test(evidence) || CONCRETE_RUGBY.test(evidence) || VENUE_WORDS.test(evidence) || QUOTED.test(evidence), matchLike = MATCHISH.test(titleEvidence), hasMatchContext = !matchLike || SCORE_OR_NUMBER.test(evidence) || MONTH.test(evidence) || VENUE_WORDS.test(evidence) || /\b(?:first|second|third|fourth) test\b/i.test(evidence), completedMatch = COMPLETED_MATCH.test(titleEvidence), squadSelection = SQUAD_SELECTION.test(titleEvidence);
  const squadIdentitySupported = squadSelection && factPeople.length >= 2 && distinctPublishers.size >= 3;
  const factIdentitySupported = factPeople.some((person) => titleSupportsFactPerson(titleEvidence, person)) && distinctPublishers.size >= 2 && distinctFacts.length >= 2;
  const identitySupported = corroboratedPeople.length >= 1 || squadIdentitySupported || factIdentitySupported;
  const reasons = [];
  if (EXPLICIT_NON_RUGBY_TITLE.test(titleEvidence)) reasons.push("explicit-non-rugby-title");
  ...
  if (names.length >= 1 && !identitySupported) reasons.push("no-coherent-person-identity-support");
  ...
  const evidenceScore = Math.min(4, distinctPublishers.size) * 5 + Math.min(8, distinctFacts.length) * 2 + Math.min(4, directNames.length) * 4 + Math.min(3, corroboratedPeople.length) * 4 + (squadIdentitySupported ? 3 : 0) + (factIdentitySupported ? 2 : 0) + (FINAL_SCORE.test(factEvidence) ? 4 : 0) + (MONTH.test(evidence) ? 2 : 0) + (VENUE_WORDS.test(evidence) ? 2 : 0) + (CONCRETE_RUGBY.test(evidence) ? 3 : 0);
  return { passed: reasons.length === 0, reasons, namedPeople: names.slice(0, 8), directNamedPeople: directNames.slice(0, 8), corroboratedPeople: corroboratedPeople.slice(0, 8), factIdentitySupported, squadIdentitySupported, sourceCount: sources.length, editorialNewsSourceCount: editorialSources.length, distinctPublisherCount: distinctPublishers.size, factCount: facts.length, distinctFactCount: distinctFacts.length, hasConcreteMarker, matchLike, hasMatchContext, completedMatch, squadSelection, factNamedPeople: factPeople.slice(0, 8), evidenceScore };
}
const assessed = raw.candidates.map((candidate, index) => ({ candidate, index, assessment: assess(candidate) }));
const acceptedAssessments = assessed.filter(({ assessment }) => assessment.passed).sort((left, right) => right.assessment.evidenceScore - left.assessment.evidenceScore || left.index - right.index), accepted = acceptedAssessments.map(({ candidate }) => candidate), rejected = assessed.filter(({ assessment }) => !assessment.passed).map(({ candidate, assessment }) => ({ id: candidate.id, title: candidate.title, ...assessment }));
