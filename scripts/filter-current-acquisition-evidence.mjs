...
const NON_PERSON_NAMES = new Set(["Planet Rugby","Irish Rugby","Rugby Pass","RugbyPass Ireland","Business Post","The Irish Times","The Guardian","United Rugby","Rugby Football","Football Union","United Rugby Championship","Irish Independent","Connacht Rugby","Munster Rugby","Leinster Rugby","Ulster Rugby","England Rugby","Red Roses","WXV Global","WXV Global Series","Global Series","Wallaroos Global","Western Force","Force Force","Roster Confirmed","ROSTER CONFIRMED","WXV Match","WXV Series","Two-time Olympic"]);
...
function assess(candidate) {
  ...
  if(names.length>=1&&corroboratedPeople.length<1&&!squadIdentitySupported) reasons.push("no-person-identity-corroborated-across-sources");
  ...
  const evidenceScore=Math.min(4,distinctPublishers.size)*5+Math.min(8,distinctFacts.length)*2+Math.min(4,directNames.length)*4+Math.min(3,corroboratedPeople.length)*4+(squadIdentitySupported?3:0)+(FINAL_SCORE.test(factEvidence)?4:0)+(MONTH.test(evidence)?2:0)+(VENUE_WORDS.test(evidence)?2:0)+(CONCRETE_RUGBY.test(evidence)?3:0);
  return {passed:reasons.length===0,reasons,namedPeople:names.slice(0,8),directNamedPeople:directNames.slice(0,8),corroboratedPeople:corroboratedPeople.slice(0,8),squadIdentitySupported,sourceCount:sources.length,editorialNewsSourceCount:editorialSources.length,distinctPublisherCount:distinctPublishers.size,factCount:facts.length,distinctFactCount:distinctFacts.length,hasConcreteMarker,matchLike,hasMatchContext,completedMatch,squadSelection,factNamedPeople:factPeople.slice(0,8),evidenceScore};
}
