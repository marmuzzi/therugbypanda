...
const lookbackHours = Number.parseInt(process.env.IMAGE_ARTICLE_LOOKBACK_HOURS ?? "18", 10);
...
const TEAMS = ["Leinster", "Munster", "Ulster", "Connacht", "Ireland"];
...
function scoreCandidate(article, image) {
  ...
  const storyTeam = TEAMS.find((team) => storyLower.includes(team.toLowerCase()));
  const imageTeams = TEAMS.filter((team) => meta.includes(team.toLowerCase()));
  if (storyTeam && imageTeams.length && !imageTeams.includes(storyTeam)) return Number.NEGATIVE_INFINITY;
  ...
  const teamMatch = storyTeam && meta.includes(storyTeam.toLowerCase());
...
function acquisitionQueries(article) {
  ...
  for (const person of people) {
    queries.push(`${person} rugby 2026`, `${person} ${team ?? "Ireland"} rugby`, `${person} rugby 2025`);
  }
...
const articles = await query(`*[_type == "article" && _id in path("drafts.**") && morningPackageEligible == true && _updatedAt >= $cutoff] | order(_updatedAt desc)[0...5]{
...
if (articles.length !== 5) throw new Error(`Expected exactly five recent morning-package drafts, found ${articles.length}. Fail closed.`);
if (new Set(articles.map((x) => x.editorialInputId).filter(Boolean)).size !== 5) throw new Error("Morning-package drafts do not have five distinct editorialInputId values. Fail closed.");
...
console.log(JSON.stringify({articleCount:result.articleCount,articlesMeetingTarget:result.articlesMeetingTarget,totalLocalCandidates:result.totalLocalCandidates,totalDeficit:result.totalDeficit},null,2));
