...
const retainedRaw=await client.fetch(`*[_type == "article" && _id in path("drafts.**") && morningPackageEligible == true && coalesce(automationContentClass, "production") == "production"] {editorialInputId}`);
const retainedIds=new Set((Array.isArray(retainedRaw)?retainedRaw:[]).map((d)=>d.editorialInputId).filter((id)=>isCurrentPackageEditorialInputId(id,packageDate)));
...
// The daily budget ledger is authoritative and gives us a durable, cross-run exclusion list.
...
const available=batch.candidates.filter((c)=>!retainedIds.has(c.id)&&!excludedIds.has(c.id));
