...
const affordableSlots=Math.max(0,Math.floor((normalTargetHeadroomUsd+1e-9)/RESERVATION_PER_SLOT_USD));
const slotsToPlan=Math.min(missingSlots,affordableSlots);
if(missingSlots>0&&slotsToPlan===0)throw new Error(`Slot-budget planning normal-target exhausted: $${reservedUsd.toFixed(3)} already reserved leaves no $${RESERVATION_PER_SLOT_USD.toFixed(3)} slot inside the $${NORMAL_TARGET_USD.toFixed(2)} Europe/Dublin normal target.`);
const irishNeededThisPass=Math.min(irishNeeded,slotsToPlan);
