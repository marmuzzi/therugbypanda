...
const minCandidateScore = Math.max(0, Number(process.env.MIN_CANDIDATE_SCORE ?? 100));
const excludedDraftIds = new Set(["drafts.article-welcome-to-the-rugby-panda"]);
...
const summary = { apply, scannedDrafts: drafts.length, libraryImages: images.length, minCandidateScore, updated: 0, unchanged: 0, noSafeMatch: 0, weakMatch: 0, excluded: 0, drafts: [] };

for (const article of drafts) {
  if (excludedDraftIds.has(article._id)) {
    summary.excluded++;
    summary.drafts.push({ id: article._id, title: article.title, action: "excluded-owned-editorial" });
    continue;
  }
  ...
  if (best.score < minCandidateScore) { record.action = "weak-match"; summary.weakMatch++; summary.drafts.push(record); continue; }
  const shouldReplace = !Number.isFinite(currentScore) || best.score >= currentScore + minImprovement;
  ...
  const inlineCandidates = ranked.filter(x=>x.image.assetRef !== best.image.assetRef && x.score >= minCandidateScore).slice(0,2).map(x=>x.image);
...
console.log(JSON.stringify({ scannedDrafts: summary.scannedDrafts, libraryImages: summary.libraryImages, updated: summary.updated, unchanged: summary.unchanged, noSafeMatch: summary.noSafeMatch, weakMatch: summary.weakMatch, excluded: summary.excluded, apply }));
