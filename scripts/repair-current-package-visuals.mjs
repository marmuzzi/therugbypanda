...
function groupsIn(value) {
  ...
  return TEAM_GROUPS.filter((group) => group.terms.some((term) => lower.includes(term))).map((group) => group.id);
}
function words(value) {
  return [...new Set(String(value ?? "").toLowerCase().split(/[^a-z0-9à-öø-ÿ]+/).filter((word) => word.length >= 5 && !GENERIC.has(word)))];
}
function namedPeople(value) {
  const matches = String(value ?? "").match(/\b[A-Z][A-Za-zÀ-ÖØ-öø-ÿ'’.-]+\s+[A-Z][A-Za-zÀ-ÖØ-öø-ÿ'’.-]+\b/g) ?? [];
  return [...new Set(matches.filter((name) => !TEAM_GROUPS.some((group) => group.terms.includes(name.toLowerCase()))))];
}
function portableImage(image) {
  return {
    _type: "image", _key: crypto.randomUUID().replaceAll("-", "").slice(0, 12),
...
function relevance(article, image, paragraph = null) {
  const header = `${article.title ?? ""} ${article.standfirst ?? ""}`;
  const body = (article.body ?? []).filter((b) => b?._type === "block").map(blockText).join(" ");
  const story = `${header} ${body}`;
  const meta = imageText(image);
  if (!image.assetRef || NON_RUGBY.test(meta)) return -Infinity;

  // Critical launch rule: if an image declares a national/provincial team, it must belong to a team named in the headline/standfirst.
  const primaryTeams = groupsIn(header);
  const imageTeams = groupsIn(meta);
  if (imageTeams.length && (!primaryTeams.length || !imageTeams.some((team) => primaryTeams.includes(team)))) return -Infinity;

  const storyLower = story.toLowerCase();
  const imagePeople = namedPeople(meta);
  if (imagePeople.some((person) => !storyLower.includes(person.toLowerCase()))) return -Infinity;
  const womenStory = /\bwomen(?:'s)?\b|\bfemale\b|\bgirls\b/i.test(header);
  const womenImage = /\bwomen(?:'s)?\b|\bfemale\b|\bgirls\b/i.test(meta);
  if (womenStory !== womenImage) return -Infinity;
  ...
  const teamMatches = groupsIn(target).filter((team) => imageTeams.includes(team));
  const shared = words(target).filter((word) => meta.toLowerCase().includes(word));
...
const articles = await query(`*[_type == "article" && _id in path("drafts.**") && morningPackageEligible == true && automationContentClass == "production" && editorialInputId match $prefix] | order(_updatedAt desc)[0...5]{_id,title,standfirst,body,editorialInputId}`, { prefix });
...
for (const article of articles) {
  const rankedHero = images.map((image) => ({ image, score: relevance(article, image) }))
    .filter(({ image, score }) => !globallyUsed.has(image.assetRef) && Number.isFinite(score) && score >= 45)
    .sort((a,b) => b.score - a.score);
  const hero = rankedHero[0];
  if (!hero) throw new Error(`No strongly relevant rights-approved hero for ${article.title}.`);
  globallyUsed.add(hero.image.assetRef);

  const cleanBody = (article.body ?? []).filter((block) => block?._type !== "image");
  const selections = [];
  for (const block of cleanBody) {
    if (block?._type !== "block" || block.style === "h2") continue;
    const paragraph = blockText(block);
    if (!paragraph) continue;
    const ranked = images.map((image) => ({ image, score: relevance({ ...article, body: cleanBody }, image, paragraph) }))
      .filter(({ image, score }) => !globallyUsed.has(image.assetRef) && image.assetRef !== hero.image.assetRef && Number.isFinite(score) && score >= 45)
      .sort((a,b) => b.score - a.score);
    if (ranked[0]) { selections.push({ ...ranked[0], afterKey: block._key }); globallyUsed.add(ranked[0].image.assetRef); }
    if (selections.length === 2) break;
  }
  if (selections.length < 1) throw new Error(`No meaningful inline image for ${article.title}; refusing filler.`);

  const body = [];
  for (const block of cleanBody) {
    body.push(block);
    const selected = selections.find((item) => item.afterKey === block?._key);
    if (selected) body.push(portableImage(selected.image));
  }
  const featuredImage = portableImage(hero.image); delete featuredImage._key;
  await mutate([{ patch: { id: article._id, set: { featuredImage, body } } }]);
  summary.push({ editorialInputId: article.editorialInputId, title: article.title, hero: { title: hero.image.title, team: hero.image.team, score: hero.score }, inline: selections.map((x) => ({ title: x.image.title, team: x.image.team, score: x.score })) });
}

const verify = await query(`*[_type == "article" && _id in path("drafts.**") && editorialInputId match $prefix] | order(_updatedAt desc)[0...5]{editorialInputId,title,"hero":featuredImage.asset._ref,"inline":body[_type == "image"].asset._ref}`, { prefix });
for (const article of verify) {
  if (!article.hero || (article.inline?.length ?? 0) < 1) throw new Error(`Readback failed for ${article.title}.`);
}
