...
function imageText(image) { return [image.title,image.altText,image.alt,image.caption,image.subject,image.team,image.competitionEvent].filter(Boolean).join(" "); }
...
function explicitPeople(image) { return Array.isArray(image.people) ? image.people.filter(Boolean) : []; }
function negativeConflict(article, image) {
  ...
  // A multi-team image is safe only when every recognised team belongs to the article's primary context.
  // This deliberately rejects France-v-South-Africa imagery for a South-Africa-v-New-Zealand story.
  if (imageTeams.some((team) => !primaryTeams.includes(team))) return true;
  if (explicitPeople(image).some((person) => !story.toLowerCase().includes(String(person).toLowerCase()))) return true;
...
function relevance(article, image, paragraph = null) {
  ...
  const exactPeople = explicitPeople(image).filter((person) => targetLower.includes(String(person).toLowerCase()));
...
for (const article of articles) {
  ...
  summary.push({ editorialInputId: article.editorialInputId, title: article.title, hero: { title: hero.image.title, team: hero.image.team, people: hero.image.people, assetRef: hero.image.assetRef, score: hero.score, retained: Boolean(hero.retained) }, inline: selections.map((x) => ({ title: x.image.title, team: x.image.team, people: x.image.people, assetRef: x.image.assetRef, score: x.score })), inlineCount: selections.length });
}
